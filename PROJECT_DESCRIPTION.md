...
| **Verifiable usage record #4 — live multi-caller usage log** | `verifiable_usage_records/live-usage-latest.md` (+ `live-usage-latest.json`) — 34 real calls into joulyzer from three callers (CLI subprocess, in-process function, JSON-RPC MCP subprocess) + the shipped demo run + three live `api.bitget.com` calls. Session header (id, started, ended, duration), per-caller summary, numbered timeline with **Request** / **Response** JSON blocks. Per-record shape includes `session_id` field, matching `hasbunallah01/quant-copilot/logs/live-usage-latest.md`. Full per-file manifest: `docs/verifiable-usage-records.md`. |
...
| **Verifiable usage record #6 — focused sample I/O** | `verifiable_usage_records/sample-api-io.md` — 5 focused request/response pairs (one per caller + the demo run + one live Bitget call). |
| **Verifiable usage record #7 — raw live Bitget transcript** | `verifiable_usage_records/live-bitget-server-time.txt` — live HTTP call to `api.bitget.com/api/v2/public/time` with response headers verbatim. |
| **Verifiable usage record #8 — integration example** | `examples/agent_integration.py` + `scripts/mcp_client_demo.py` + `scripts/live_usage_harness.py` (another developer can run them via `bash scripts/verify_submission.sh`) |
