...
| **Verifiable usage record #4 — live multi-caller usage log** | `verifiable_usage_records/live-usage-latest.md` (+ `live-usage-latest.json`) — 33 real calls into joulyzer from three callers (CLI subprocess, in-process function, JSON-RPC MCP subprocess) + three live `api.bitget.com` calls. Session header (id, started, ended, duration), per-caller summary, numbered timeline with **Request** / **Response** JSON blocks. Pattern matches `hasbunallah01/quant-copilot/logs/live-usage-latest.md`. |
